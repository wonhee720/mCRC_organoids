...
option_list = list(
    ...
    make_option(c("-r","--header"), type="character",
                ...
                help="[Optional] A column header names, comma-separated, used when the input is of table form instead of vcf (default: 'Substitution,Trinucleotide')",metavar="character"), 
    ...
    make_option(c("-n","--signature_number"), type="character",
                default="v2_all",
                help="[Optional] Signature numbers to fit, comma-separated or colon-ranged or combination of the two. A special inputs: 'v2_all', 'v3_all', or 'v2_all__v3_all'. For --algorithm 4, the fitting is done in the order specified here (e.g., v2_5,1,7 will fit with 5 first and then progressively 1 and 7) (default: 'v2_all'). The syntax is \'v2_<signature_numbers>__v3_<signature_numbers>\' where __ is double underscore (e.g. v2_1,2:5__v3_1:4,7a,7b). Mixing with 'all' is allowed (e.g. v2_all_v3_1:5).",metavar="character"), 
    make_option(c("-g","--refGenome"), type="character",
                default="hg19",
                help="[Optional] a reference genome name (supported: hg19, hg38, mm10; default: hg19)",metavar="character"),
    ...
    make_option(c("-l","--list"), action="store_true", 
                default=FALSE, 
                help="[Optional] If this flag is used, -i (--input) takes a text file as an input that contains a list of arbitrarily many input file paths. These input files are processed and merged into one sample (i.e., MISO and not MIMO)."), 
...
run <- function(){
  ...
  if (signature_chunking & (opt$algorithm != 4)) {
    ...
    signatureNames <- tmp.signature_number %>% lapply(function(x){
      x <- tolower(x)
      x.header <- x %>% gsub("(^(v2_)|^(v3_)).*","\\1",.)
      x.body <- x %>% gsub("^(v2_|v3_)(.*)","\\2",.)
  ...
  infer_inputType_and_get_mutDF <- function(inputFile = NULL, inputType = NULL){
    if (is.null(inputType)) {
      ...
      if (!is.vcf & !is.tsv){
        system("echo -e \"\\e[38;5;1mERROR: Unrecognizable input types (vcf or table). Please check the data formats.\\e[0m\"")
        stop_quietly()  
      } 
      if (is.vcf == TRUE) {
  ...
  # HELPER FUNCTION: plotTotalCounts 
	...
	generate_and_save_plots <- function(sigAn.results.list=NULL, mutDF=NULL, sigDF=NULL, mergeData=FALSE){
	  for (i in 1:length(sigAn.results.list)){
	    ...
	    exp <- rbind(exp, data.frame(Signature = "Unexplained", exp = res$unexplained, stringsAsFactors=FALSE))
	    exp$prop <- if(sum(res$x)!=0){round(exp$exp/res$pseudo_totalCount*100, 2)}else{NA} 
	    ...
	    system(paste0("echo -ne \"\\e[38;5;15mSaving files...(",i,"/",length(sigAn.results.list),")\\r\"")) 
	    suppressWarnings(p.m <- plotMutationalSpectrum(mutDF.i))
	    # EXCEPTION HANDLING EXCEPTION HANDLING EXCEPTION HANDLING, 2019-12-16

...
def sample_decomposition(sample, cancer="", signatures=None, mtype="", genome_build="GRCh37", add_penalty=0.05, radiation=False, brca=False, remove_penalty=0.01, mutation_context=None, connected_sigs=True, make_decomposition_plots=True, originalProcessAvg=None):
    '''
    ...
    if sample.shape[0]==96:
        mtype = '96'
        if (genome_build=="GRCh37") and (cancer==""):
        ...
        elif (genome_build=="GRCh37") and (cancer=="Colon") and (brca==False):
            sigDatabase = pd.read_excel(paths+"/data/SBS_signatures_genome_builds.xlsx", sheet_name="GRCh37-CRC")
            sigDatabase,_,_,_ = sub.read_csv(sigDatabase) 
        elif (genome_build=="GRCh37") and (cancer=="Colon-MSI") and (brca==False):
            sigDatabase = pd.read_excel(paths+"/data/SBS_signatures_genome_builds.xlsx", sheet_name="GRCh37-CRC-MSI")
            sigDatabase,_,_,_ = sub.read_csv(sigDatabase)     
        elif (genome_build=="GRCh37") and (cancer=="Colon") and (brca==True):
            sigDatabase = pd.read_excel(paths+"/data/SBS_signatures_genome_builds.xlsx", sheet_name="GRCh37-CRC-brca")
            sigDatabase,_,_,_ = sub.read_csv(sigDatabase)    
    ...
    elif ((sample.shape[0]==83) & (radiation==False) & (cancer=="") & (brca==False)):
        mtype = '83'
        sigDatabase = pd.read_csv(paths+"/data/sigProfiler_ID_signatures.csv", sep=",",index_col=0)
        signames = sigDatabase.columns
        connected_sigs=False

    elif ((sample.shape[0]==83) & (radiation==False) & (cancer=="Colon") & (brca==False)):
        mtype = '83'
        sigDatabase = pd.read_csv(paths+"/data/sigProfiler_ID_signatures_colorectal.csv", sep=",",index_col=0)
        signames = sigDatabase.columns
        connected_sigs=False

    elif ((sample.shape[0]==83) & (radiation==False) & (cancer=="Colon") & (brca==True)):
        mtype = '83'
        sigDatabase = pd.read_csv(paths+"/data/sigProfiler_ID_signatures_colorectal_brca.csv", sep=",",index_col=0)
        signames = sigDatabase.columns
        connected_sigs=False

    elif ((sample.shape[0]==83) & (radiation==True) & (cancer=="") & (brca==False)):
        mtype = '83'
        sigDatabase = pd.read_csv(paths+"/data/sigProfiler_ID_signatures_radiation.csv", sep=",",index_col=0)
        signames = sigDatabase.columns
        connected_sigs=False
       
    elif ((sample.shape[0]==83) & (radiation==True) & (cancer=="Colon") & (brca==False)):
        mtype = '83'
        sigDatabase = pd.read_csv(paths+"/data/sigProfiler_ID_signatures_radiation.colorectal.csv", sep=",",index_col=0)
        signames = sigDatabase.columns
        connected_sigs=False
